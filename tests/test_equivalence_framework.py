...
class TestArithmeticOperations(EquivalenceTest):
    ...
    def test_addition_scalar(self):
        ...
        self.run_comparison(pandas_op=lambda df: df + 10, jaxframe_op=lambda df: df + 10, data=data)

    def test_subtraction_scalar(self):
        """Test df - scalar."""
        data = {"a": [10.0, 20.0, 30.0], "b": [40.0, 50.0, 60.0]}
        self.run_comparison(pandas_op=lambda df: df - 5, jaxframe_op=lambda df: df - 5, data=data)

    def test_multiplication_scalar(self):
        """Test df * scalar."""
        data = {"a": [1.0, 2.0, 3.0], "b": [4.0, 5.0, 6.0]}
        self.run_comparison(pandas_op=lambda df: df * 2, jaxframe_op=lambda df: df * 2, data=data)
...
class TestAggregations(EquivalenceTest):
    ...
    def test_sum_axis0(self):
        ...
        data = {"a": [1.0, 2.0, 3.0], "b": [4.0, 5.0, 6.0]}
        self.run_comparison(
            pandas_op=lambda df: df.sum(axis=0), jaxframe_op=lambda df: df.sum(axis=0), data=data
        )
    ...
    def test_sum_axis1(self):
        ...
        data = {"a": [1.0, 2.0, 3.0], "b": [4.0, 5.0, 6.0]}
        self.run_comparison(
            pandas_op=lambda df: df.sum(axis=1), jaxframe_op=lambda df: df.sum(axis=1), data=data
        )
    ...
    def test_mean_axis0(self):
        ...
        data = {"a": [1.0, 2.0, 3.0], "b": [4.0, 5.0, 6.0]}
        self.run_comparison(
            pandas_op=lambda df: df.mean(axis=0), jaxframe_op=lambda df: df.mean(axis=0), data=data
        )
...
class TestPropertyBasedEquivalence:
    ...
    def generate_random_data(self, n_rows=10, n_cols=3, seed=42):
        ...
        np.random.seed(seed)
        return {f"col_{i}": np.random.randn(n_rows) for i in range(n_cols)}
